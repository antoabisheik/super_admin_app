...
function Layouts() {
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://sbackend.duckdns.org/api'}/organizations`, {
        headers: {
  ...
  const fetchGyms = async (organizationId) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://sbackend.duckdns.org/api'}/organizations/${organizationId}/gyms`, {
        headers: {
          'Authorization': `Bearer ${await getAuthToken()}`
        }
      });
